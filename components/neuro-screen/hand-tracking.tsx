...
import { Hand, CheckCircle2, Loader2, AlertTriangle } from "lucide-react"
 ...
 *
 * Fixes:
 * - Sequencing uses expectedPoseRef (prevents RAF stale-state bug).
...
function palmNormalRatios(lm: MPPoint[], mirrored: boolean) {
  ...
  const z5 = lm[5].z
  const z17 = lm[17].z
  if (typeof z0 !== "number" || typeof z5 !== "number" || typeof z17 !== "number") {
    return null
  }
 ...
 * - EDGE tends to look narrower (palmWidth2D smaller)
 * - PALM DOWN tends to look wider (but not always)
 */
function classifyPose(lm: MPPoint[], mirrored: boolean): Pose {
  ...
  if (ratios) {
    ...
    // Ambiguous: small fallback using 2D width
    // (Palm-down often looks wider than edge)
...
export function HandTracking({
  ...
  const [status, setStatus] = useState<"loading" | "ready" | "tracking" | "done">(
    "loading"
  )
  ...
  // ✅ crucial: RAF loop reads this ref (not stale React state)
  ...
  const finishTask = useCallback(() => {
    ...
    const coverage = detectedFramesRef.current / Math.max(1, totalFramesRef.current)
  ...
  const startTracking = useCallback(() => {
    ...
    const processFrame = () => {
      ...
      try {
        ...
        // Overlay (do not mirror here; video is already mirrored via CSS)
        const canvas = overlayCanvasRef?.current
        if (canvas) {
          const ctx = canvas.getContext("2d")
          if (ctx) {
        ...
        const debounceFrames = pose === "PALM" ? DEBOUNCE_FRAMES_PALM : DEBOUNCE_FRAMES_DEFAULT
  ...
  return (
    <Card className="border-border bg-card p-6">
      <h2 className="mb-1 text-xl font-semibold text-foreground">Motor Sequencing Task</h2>
      ...
      {status === "ready" && (
        <Button className="w-full gap-2" onClick={startTracking} disabled={!canStart}>
